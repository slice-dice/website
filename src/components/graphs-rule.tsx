'use client'

import { ChevronDown } from 'lucide-react'
import { useState } from 'react'

export function GraphRule({ children }: { children: React.ReactNode }) {
    const [collapsed, setCollapsed] = useState<boolean>(false)

    return (
        <div className="mb-8">
            <div>
                <button
                    className="transition-color flex w-full cursor-pointer items-center justify-between rounded-lg bg-[#094f56] px-4 py-2 text-left text-white duration-300 ease-in-out hover:bg-[#0c6b75] focus:outline-hidden"
                    onClick={() => setCollapsed(!collapsed)}
                >
                    <h3 className="text-lg font-bold">Regole</h3>
                    <ChevronDown
                        className={`transform ${collapsed ? 'rotate-180' : 'rotate-0'} transition-transform duration-300 ease-in-out`}
                    />
                </button>
            </div>
            <div
                className={`transition-max-height w-full overflow-hidden duration-500 ease-in-out ${collapsed ? 'max-h-screen' : 'max-h-0'}`}
            >
                {children}
            </div>
        </div>
    )
}
